...
class RheinTester {
    ...
    def testFilterPrimitive(): Unit = {
        ...
        val listener = event.filter(c => Character.isUpperCase(c)).listen(x => out += x)
    ...
    def testLiftPrimitive(): Unit = {
        ...
        val listener = behaviourSink.lift(cell, (x: Int, y: Int) => x + y).changes().listen(x => out += x)
    ...
    def testSnapshotPrimitive(): Unit = {
        ...
        val out = new ListBuffer[String]()
        val listener = event.snapshot[Int, String](behaviourSink, (x: Long, y: Int) => x + " " + y).listen(x => out += x)
        event.send(100L)
        behaviourSink.send(2)
        event.send(200L)
